...
export function ReviewsSection({businessId,ownerUserId}:{businessId:number;ownerUserId:number}) {
  ...
  const save=useMutation({mutationFn:({rating,comment}:{rating:number;comment:string|null})=>mine.data?reviewApi.update(businessId,{rating,comment}):reviewApi.create(businessId,{rating,comment}),onSuccess:async()=>{await refresh();notify("نظر شما ثبت شد و پس از تأیید مدیر نمایش داده می‌شود.","ok")},onError:error=>notify(errorMessage(error),"danger")});
  ...
  const publicReviews=reviews.data?.reviews.filter(review=>review.id!==mine.data?.id)??[];
  return <section className="reviews-section container" aria-labelledby="reviews-title">
    ...
    <div className="reviews-list">{mine.data&&<ReviewCard review={mine.data} mine onEdit={()=>setEditing(true)} onDelete={()=>remove.mutate()} deleting={remove.isPending}/>} {publicReviews.slice(0,visibleCount).map(review=><ReviewCard key={review.id} review={review}/>)}</div>
    {publicReviews.length>visibleCount&&<button type="button" className="btn btn-secondary btn-sm reviews-show-more" onClick={()=>setVisibleCount(count=>count+6)}>نمایش نظرات بیشتر</button>}
