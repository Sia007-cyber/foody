...
export function CustomerHome({
  ...
  onSearchChange,
  showHero = true,
  showDiscoveryContent = true,
  typeFilter,
  onTypeFilterChange,
}: CustomerHomeProps) {
  const { user } = useAuth();
  const navigate = useNavigate();
  const hasCustomerWallet =
    ...
    <div className="customer-home container">
      {showHero && (
        <section className="home-hero">
          <div
            className="home-hero-orb home-hero-orb-violet"
            aria-hidden="true"
          />
          <div
            className="home-hero-orb home-hero-orb-pistachio"
            aria-hidden="true"
          />
          <div className="home-hero-stickers" aria-hidden="true">
            {HOME_HERO_STICKERS.map((s, i) => (
              <span
                key={s.key}
                className={`home-hero-sticker home-hero-sticker-${s.tier} home-hero-sticker-${i + 1}`}
              >
                {s.emoji}
              </span>
            ))}
          ...
          <div className="home-hero-content">
            ...
            <h1 className="home-hero-title">
              بهتـرین کافـه و فسـت‌فود ها همین‌جان
            </h1>
            <p className="home-hero-subtitle">
              کافه و فست‌فودها رو پیدا کن، منوها رو ببین یا میز رزرو کن.
            </p>
      ...
      {showDiscoveryContent && hasCustomerWallet && (
        <button
          type="button"
        ...
        >
          ...
          <span className="wallet-preview-right">
            {!walletsQuery.isLoading && wallets.length > 0 && (
              <span className="wallet-preview-amount">
        ...
        <section className="quick-actions">
          {quickActions.map((a) => (
            <button
              key={a.key}
              type="button"
              className={`quick-action quick-action-accent-${a.accent}`}
              onClick={a.onClick}
            >
              <span
                className={`quick-action-icon ${a.accent !== "ember" ? `quick-action-icon-${a.accent}` : ""}`}
              >
                {a.icon}
              </span>
      ...
      {showDiscoveryContent && topRatedBusinesses.length > 0 && (
        <section
          ...
          aria-labelledby="top-rated-businesses-title"
        >
      ...
      {showDiscoveryContent && featuredBusinesses.length > 0 && (
        <section
          className="nearby-section"
          aria-labelledby="featured-businesses-title"
        >
          <div className="nearby-section-head">
            <div>
      ...
      {showDiscoveryContent && topProducts.length > 0 && (
        ...
        >
          <div className="nearby-section-head">
            <div>
              <span className="section-eyebrow">محبوب نزد مشتری‌ها</span>
              <h2 id="top-products-title">محصولات برتر</h2>
            </div>
          </div>
          <div className="product-discovery-scroll">
