...
### بک‌اند
- [ ] ماژول `wallet` — جدول موجودی هر کاربر + جدول تاریخچه‌ی تراکنش (شارژ/کسر/پاداش)
  - [ ] `GET /api/wallet` (موجودی فعلی + سقف اعتباری قابل خرید)
  - [ ] `POST /api/wallet/topup` (شارژ دستی، بدون درگاه واقعی)
  - [ ] `GET /api/wallet/transactions`
  - [ ] کسر خودکار از کیف پول حین ثبت سفارش (اختیاری/در کنار پرداخت درب منزل)
...
- [ ] فیلد امتیاز (rating) و فاصله (distance) روی کارت کسب‌وکار در هوم — امتیاز засед فعلاً دستی/seed (ماژول reviews واقعی فاز ۳ هست)، فاصله بر پایه‌ی lat/lng کسب‌وکار و موقعیت کاربر
