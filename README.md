...
- [x] جلوگیری از startup پروفایل `prod` با JWT secret مفقود، خالی، fallback توسعه، Base64 نامعتبر یا کلید HMAC کوتاه‌تر از ۲۵۶ بیت.
- [ ] بررسی حساب‌های seed و مقادیر واقعی محیط منتشرشده. وضعیت deployment زنده در این دور بررسی نشده است.
...
- `server.port` از `PORT` خوانده می‌شود. `FOODY_JWT_SECRET` در پروفایل `prod` الزامی است، باید Base64 باشد و پس از decode حداقل ۳۲ بایت داشته باشد؛ startup مقدار خالی، نامعتبر و fallback شناخته‌شدهٔ توسعه را رد می‌کند. یک مقدار مناسب را با `openssl rand -base64 32` تولید و فقط در secret محیط انتشار ذخیره کنید.
