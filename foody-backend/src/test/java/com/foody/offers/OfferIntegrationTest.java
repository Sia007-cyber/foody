...
class OfferIntegrationTest extends AbstractContainerBaseTest {
    ...
    @Test void customerClaimsActiveOfferOnce(){OfferResponse o=create(owner,2);OfferClaimResponse c=service.claim(customer.getId(),o.id());assertThat(c.offerId()).isEqualTo(o.id());assertThat(c.offerTitle()).isEqualTo("Limited offer");assertThat(c.businessName()).isEqualTo(cafe.getName());assertThat(c.remainingAvailability()).isEqualTo(1);}
    @Test void businessOwnerClaimsAnotherBusinessOffer(){OfferResponse o=create(owner,2);assertThat(service.claim(otherOwner.getId(),o.id()).offerId()).isEqualTo(o.id());}
    @Test void businessOwnerCannotClaimOwnOffer(){OfferResponse o=create(owner,2);assertThatThrownBy(()->service.claim(owner.getId(),o.id())).isInstanceOf(org.springframework.security.access.AccessDeniedException.class);}
    ...
    @Test void exhaustedOfferLeavesDiscoveryButRemainsInCustomerHistory(){OfferResponse o=create(owner,1);service.claim(customer.getId(),o.id());assertThat(service.claimableOffers()).extracting(OfferResponse::id).doesNotContain(o.id());assertThat(service.myClaims(customer.getId())).singleElement().satisfies(c->{assertThat(c.offerTitle()).isEqualTo("Limited offer");assertThat(c.businessName()).isEqualTo(cafe.getName());assertThat(c.remainingAvailability()).isZero();});}
    @Test void ownerCanSeeClaimingCustomerForOwnedOfferOnly(){OfferResponse o=create(owner,2);service.claim(customer.getId(),o.id());assertThat(service.ownerClaims(owner.getId(),o.id())).singleElement().satisfies(c->{assertThat(c.customerPublicId()).isEqualTo(customer.getPublicId());assertThat(c.customerDisplayName()).isEqualTo(customer.getFullName());});assertThatThrownBy(()->service.ownerClaims(otherOwner.getId(),o.id())).isInstanceOf(ResourceNotFoundException.class);}
