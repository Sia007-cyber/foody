...
public interface OwnerDebitRequestRepository extends JpaRepository<OwnerDebitRequest,Long>{
 ...
 List<OwnerDebitRequest> findByCustomerUserIdAndStatusOrderByResolvedAtDesc(Long id,DebitRequestStatus status);
 List<OwnerDebitRequest> findByBusinessIdAndStatusOrderByResolvedAtDesc(Long id,DebitRequestStatus status);
